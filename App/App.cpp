...
int main(int argc, char *argv[]) /* not using since just testing w/ wc_test */
{
    try{
        ...
        auto file_sink = std::make_shared<spdlog::sinks::daily_file_sink_mt>("/data/kms/logs/exec.log", 0, 1);
    ...
    ret = sgx_create_enclave(ENCLAVE_FILENAME, DEBUG_VALUE, &t, &updated, &global_eid, NULL);
    if (ret != SGX_SUCCESS)
    {
    ...
    svr.Post("/accesshandler/signin", [&](const auto& req, auto& res) {
        ...
        if(res_==2){
            ...
            res.set_content("{\"status\":\"ok\",\"role\":\""+r+"\"}", "text/plain"); 
        }else if(res_==-2){ 
    ...
    svr.Post("/accesshandler/signout", [&](const auto& req, auto& res) {
        ...
        if(res_==2){
            ...
            res.set_content("{\"status\":\"failed\",\"reason\":\"user-not-signedin\"}", "text/plain");
        }else{ 
    ...
    svr.Post("/accesshandler/remove", [&](const auto& req, auto& res) {
        ...
        if(res_==2){
            ...
            res.set_content("{\"status\":\"failed\",\"reason\":\"user-not-existent\"}", "text/plain");
        }else{ 
