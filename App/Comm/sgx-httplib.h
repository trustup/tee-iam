...
#define CPPHTTPLIB_RECV_BUFSIZ size_t(32696u)
#endif
...
public:
  ...
  virtual ssize_t secure_read(char *ptr, size_t size) = 0;
  virtual ssize_t write(const char *ptr, size_t size) = 0;
  virtual ssize_t secure_write(const char *ptr, size_t size) = 0;
  ...
  ssize_t secure_write(const char *ptr);
  ssize_t secure_write(const std::string &s);
};
...
public:
  ...
  void set_exchange(const char *excng);
  void set_path(const char *c_path);
...
public:
  ...
  ssize_t secure_read(char *ptr, size_t size) override;
  ssize_t write(const char *ptr, size_t size) override;
  ssize_t secure_write(const char *ptr, size_t size) override;
...
public:
  SSLSocketStream(socket_t sock, long ssl, time_t read_timeout_sec,
                  time_t read_timeout_usec, time_t write_timeout_sec,
                  time_t write_timeout_usec,  std::string uuid, std::string exchange, std::string c_path);
  ...
  return prepare_content_receiver(
      x, status, receiver, decompress, [&](const ContentReceiver &out) {
        ...
        if (is_chunked_transfer_encoding(x.headers)) {
          ...
          if(has_header(x.headers, "Authorization")){
            std::string authtok = get_header_value(x.headers, "Authorization");
          }
...
inline std::pair<std::string, std::string> make_digest_authentication_header(
  ...
  string response;
  {
    auto H = algo == "SHA-256"
                 ? detail::SHA_256
                 : algo == "SHA-512" ? detail::SHA_512 : detail::SHA_512;
...
inline ssize_t Stream::secure_write(const char *ptr) {
  return secure_write(ptr, strlen(ptr));
}

inline ssize_t Stream::secure_write(const std::string &s) {
  return secure_write(s.data(), s.size());
}
...
inline ssize_t SocketStream::secure_read(char *ptr, size_t size) {
  if (!is_readable()) { return -1; }
...
inline ssize_t SocketStream::secure_write(const char *ptr, size_t size) {
  if (!is_writable()) { return -1; }
...
inline ssize_t BufferStream::secure_read(char *ptr, size_t size) {
#if defined(_MSC_VER) && _MSC_VER <= 1900
  auto len_read = buffer._Copy_s(ptr, size, size, position);
#else
  auto len_read = buffer.copy(ptr, size, position);
...
inline ssize_t BufferStream::secure_write(const char *ptr, size_t size) {
  buffer.append(ptr, size);
  return static_cast<ssize_t>(size);
}
...
inline bool Server::write_response(Stream &strm, bool close_connection,
  ...
  if (req.method != "HEAD") {
    if (!res.body.empty()) {
      if (!strm.secure_write(res.body)) { ret = false; }
...
inline long ssl_new(socket_t sock, long ctx , std::mutex &ctx_mutex,
                    U SSL_connect_or_accept, V setup) {
  long ssl = 0;
  {
    std::lock_guard<std::mutex> guard(ctx_mutex);
    int sgxStatus = enc_wolfSSL_new(global_eid, &ssl, ctx);
    if (sgxStatus != SGX_SUCCESS || ssl < 0) {
        std::cout<<"wolfSSL_new error"<<std::endl;
        return -1; 
    }
  }
  ...
  if (ssl>=0) {
    ...
    int ret;
    int sgxStatus = enc_wolfSSL_set_fd(global_eid, &ret, ssl, sock);
    if (sgxStatus != SGX_SUCCESS || ret != SSL_SUCCESS) {
          std::cout<<"wolfSSL_set_fd failure"<<std::endl;
        return -1;
    }

    sgxStatus= enc_wolfSSL_accept(global_eid, &ret, ssl);
    if ( sgxStatus != SGX_SUCCESS && ret < 0) {
      // SSL_shutdown(ssl);
      {
        std::cout<<"wolfSSL_accept error"<<std::endl;
        std::lock_guard<std::mutex> guard(ctx_mutex);
        enc_wolfSSL_free(global_eid, ssl);
      }
      return -1;
    }
  }
...
inline void ssl_delete(std::mutex &ctx_mutex, long ssl,
                       bool process_socket_ret) {
...
template <typename T>
inline bool
process_server_socket_ssl(long ssl, socket_t sock, size_t keep_alive_max_count,
...
template <typename T>
inline bool
process_client_socket_ssl(std::string uuid, std::string selected_exchange, std::string curr_path, long ssl, socket_t sock, time_t read_timeout_sec,
                          time_t read_timeout_usec, time_t write_timeout_sec,
                          time_t write_timeout_usec, T callback) {
  SSLSocketStream strm(sock, ssl, read_timeout_sec, read_timeout_usec,
                       write_timeout_sec, write_timeout_usec, uuid, selected_exchange, curr_path);
  return callback(strm);
}
...
inline SSLSocketStream::SSLSocketStream(socket_t sock, long ssl,
                                        time_t read_timeout_sec,
                                        time_t read_timeout_usec,
                                        time_t write_timeout_sec,
                                        time_t write_timeout_usec, std::string user, std::string exchange, std::string c_path)
...
inline ssize_t SSLSocketStream::read(char *ptr, size_t size) {
    int ret;
    enc_wolfSSL_secure_http_read(global_eid, &ret, ssl_, ptr, static_cast<int>(size));
    return ret;
}
  


inline ssize_t SSLSocketStream::secure_read(char *ptr, size_t size) {

  if (is_readable()) {
    int ret;
    // enc_wolfSSL_secure_http_read(global_eid, &ret, ssl_);
    return ret;
  }
  return -1;
}
...
inline ssize_t SSLSocketStream::write(const char *ptr, size_t size) {
  if (is_writable()) { 
    int ret;
    enc_wolfSSL_secure_http_write(global_eid, &ret, ssl_, ptr);
    return ret;
  }
  return -1;
}
...
inline ssize_t SSLSocketStream::secure_write(const char *ptr, size_t size) {
  if (is_writable()) { 
    int ret;
    enc_wolfSSL_secure_http_write(global_eid, &ret,  ssl_, ptr);
    return ret;
...
inline SSLServer::SSLServer(const char *cert_path, const char *private_key_path,
  ...
  if (sgxStatus != SGX_SUCCESS) {
      std::cout<<"wolfTLSv1_2_server_method failure"<<std::endl;
  }

  sgxStatus = enc_wolfSSL_CTX_new(global_eid, &ctx_, method);
  if (sgxStatus != SGX_SUCCESS || ctx_ < 0) {
      std::cout<<"wolfSSL_CTX_new failure"<<std::endl;
  }

  int ret;
  if (ctx_>=0) {
      sgxStatus = enc_wolfSSL_CTX_use_certificate_chain_buffer_format(global_eid, &ret, ctx_,
            server_cert_der_2048, sizeof_server_cert_der_2048, SSL_FILETYPE_ASN1, 0);
      if (sgxStatus != SGX_SUCCESS || ret != SSL_SUCCESS) {
          std::cout<<"enc_wolfSSL_CTX_use_certificate_chain_buffer_format failure "<<ret<< std::endl;
      }

      sgxStatus = enc_wolfSSL_CTX_use_PrivateKey_buffer(global_eid, &ret, ctx_,
              server_key_der_2048, sizeof_server_key_der_2048, SSL_FILETYPE_ASN1, 0); //SSL_FILETYPE_PEM
      if (sgxStatus != SGX_SUCCESS || ret != SSL_SUCCESS) {          

          std::cout<<"wolfSSL_CTX_use_PrivateKey_buffer failure"<<std::endl;
      } 
 
      // sgxStatus = enc_wolfSSL_CTX_load_verify_buffer(global_eid, &ret,
      //         ctx_, ca_cert_der_2048, sizeof_ca_cert_der_2048, SSL_FILETYPE_PEM, 1);

      // if (sgxStatus != SGX_SUCCESS || ret != SSL_SUCCESS)
      // {
      //     std::cout<<"Error loading cert"<<std::endl;
      //     ret=false;
      // }
  }
}
...
inline SSLServer::~SSLServer() {
  ...
  if (ctx_>=0) { 
    int ret;
    enc_wolfSSL_CTX_free(global_eid, ctx_);   
    // enc_wolfSSL_Cleanup(global_eid, &ret);
  }
...
inline bool SSLServer::process_and_close_socket(socket_t sock) {
  long ssl = detail::ssl_new(sock, ctx_, ctx_mutex_, SSL_accept,
                             [](long  /*ssl*/) { return true; });
...
inline SSLClient::SSLClient(const std::string &host, int port,
  ...
  sgxStatus = enc_wolfTLSv1_2_client_method(global_eid, &method);
  if (sgxStatus != SGX_SUCCESS) {
      std::cout<<"wolfTLSv1_2_client_method failure"<<std::endl;
  }
  ...
  int ret;
  // const char * defaultCipherList = "TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA";
  // int reeM;
  // sgxStatus = enc_wolfSSL_CTX_set_cipher_list(global_eid, &reeM, ctx, defaultCipherList);
  // if (sgxStatus != SGX_SUCCESS || reeM != SSL_SUCCESS) {
  //     std::cout<<"enc_wolfSSL_CTX_set_cipher_list failure"<<std::endl;
  // }

  sgxStatus = enc_wolfSSL_CTX_use_certificate_chain_buffer_format(global_eid, &ret, ctx,
      client_cert_der_2048, sizeof_client_cert_der_2048, SSL_FILETYPE_ASN1, 0);
  ...
  sgxStatus = enc_wolfSSL_CTX_use_PrivateKey_buffer(global_eid, &ret, ctx,
          client_key_der_2048, sizeof_client_key_der_2048, SSL_FILETYPE_ASN1, 0);
  if (sgxStatus != SGX_SUCCESS || ret != SSL_SUCCESS) {
      std::cout<<"wolfSSL_CTX_use_PrivateKey_buffer failure"<<std::endl; 
  }

  // sgxStatus = enc_wolfSSL_CTX_load_verify_buffer(global_eid, &ret,
  //         ctx, ca_cert_der_2048, sizeof_ca_cert_der_2048, SSL_FILETYPE_ASN1, 0);
  // if (sgxStatus != SGX_SUCCESS || ret != SSL_SUCCESS)
  // {
  //     std::cout<<"Error with cert"<<std::endl;
  //     ret=false;
  // }
  
  if (host != "www.okex.com" && host != "api.fireblocks.io"){
    int res;
    sgxStatus = enc_wolfSSL_set_SNI(global_eid, &res, ctx, host.c_str());
    if (sgxStatus != SGX_SUCCESS || res < 0) {
        std::cout<<"wolfSSL_set_SNI error"<<std::endl;
        ret=false;
    }

    sgxStatus = enc_wolfSSL_CTX_UseSupportedCurve(global_eid, &res, ctx);
    if (sgxStatus != SGX_SUCCESS || res < 0) {
        std::cout<<"wolfSSL_set_SNI error"<<std::endl;
        ret=false;
    }
...
inline void SSLClient::set_path(const char *c_path) {
  curr_path=c_path;
}

inline void SSLClient::set_exchange(const char *excng) {
  selected_exchange=excng;
}

inline void SSLClient::set_uuid(const char *user) {
  uuid=user;
}
...
inline void SSLClient::enable_server_certificate_verification(bool enabled) {
  if(!enabled){
      int ret;
      enc_wolfSSL_set_verify_none(global_eid, &ret, ctx);
  }
...
inline long SSLClient::ssl_context() const { return ctx; }
...
inline bool SSLClient::initialize_ssl(Socket &socket) {
  ...
  if (sgxStatus != SGX_SUCCESS || ssl < 0) {
      std::cout<<"wolfSSL_new error: "<<ssl<<"-"<<sgxStatus<<std::endl;
      return -1;
  }
  int ret;

  sgxStatus = enc_wolfSSL_set_fd(global_eid, &ret, ssl, socket.sock);
  if (sgxStatus != SGX_SUCCESS || ret != SSL_SUCCESS) {
        std::cout<<"wolfSSL_set_fd failure"<<std::endl;
      return -1;
  }

  sgxStatus = enc_wolfSSL_connect(global_eid, &ret, ssl);
  if (sgxStatus != SGX_SUCCESS || ret != SSL_SUCCESS) {
        std::cout<<"Failed to connect to server "<<sgxStatus<<" "<<ret<<std::endl;
        int err;
        enc_wolfSSL_get_error(global_eid, &err, ssl, ret);
        printf("errorr %d\n",err);
        exit(-1);
  }
...
SSLClient::process_socket(Socket &socket,
                          std::function<bool(Stream &strm)> callback) {
  // assert(socket.ssl); 
  return detail::process_client_socket_ssl(uuid, selected_exchange,  curr_path, 
...
inline long Client::ssl_context() const {
