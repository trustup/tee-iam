	...
	if (handler == NULL) {
		...
		try {
			...
			for(auto &curr_obj : j) {
				...
				#ifdef DEBUG_MODE
				xprintf("[KEYSTORAGE - USERS] Loading:  %s - %s",uuid.c_str(), curr_obj["key_data"].get<std::string>().c_str());
				#else
...
inline int UsersStorage::append(std::string uuid, std::string key){
	std::string filename="/SecureDBdata/users.keys";
	...
	try{
		...
		if (it != USERSENTRIES.end()){	
			xprintf("[KEYSTORAGE - USERS] ERROR The user key %s already exists - Cannot add the key",uuid.c_str());
			return -2;
		}
		...
		if (handler == NULL) {
			#ifdef DEBUG_MODE
			xprintf("[KEYSTORAGE - USERS] ERROR Opening Encrypted File during Load");
			#endif
			sgx_fclose(sgx_fopen_auto_key(filename.c_str(), "w"));
			return -1;
		}
		...
		sgx_fwrite(storage_doc.dump().c_str(), storage_doc.dump().size()+1, 1, handler);
		sgx_fflush(handler);

		xprintf("[KEYSTORAGE - USERS] Key for user %s appended!",
		 uuid.c_str());

		if (sgx_fclose(handler) == 0) {
			#ifdef DEBUG_MODE
			xprintf("[ENCLAVE] Encrypted File Successfully Closed during Append");
			#endif
		} else {
			xprintf("Error while closing file...");
		}
...
inline int UsersStorage::remove(std::string uuid_2_rm){ 
	std::string filename="/SecureDBdata/users.keys";

	auto it = USERSENTRIES.find(uuid_2_rm);
  	if (it == USERSENTRIES.end()){	
		xprintf("[KEYSTORAGE - USERS] ERROR - The user key does not exist! Cannot remove!", uuid_2_rm.c_str());
		return -2;
	}
	...
	try{
		...
		xprintf("[KEYSTORAGE - USERS] Keys removed for user: %s", uuid_2_rm.c_str());
		if (sgx_fclose(handler) == 0) {
			#ifdef DEBUG_MODE
			xprintf("[ENCLAVE] Encrypted File Successfully Closed during Append");
			#endif
...
inline bool UsersStorage::exist(std::string uuid){
	auto it = USERSENTRIES.find(uuid);
  	if (it == USERSENTRIES.end()){	
		return false;
	}else{
...
inline std::string UsersStorage::getAll(){ 
	json jdoc;
	auto allarr=json::array();
	for (auto const& pair : USERSENTRIES){
		std::string result="{\"uuid\":\""+pair.first+"\",\"key_data\":\""+pair.second+"\"}";
