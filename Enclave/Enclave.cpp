...
//curl --insecure -v -d '{"uuid": "gmazzeo","op": "signup","email":"gmazzeo@cerict.it","role":"researcher","pwd":"superpippo"}' -H "Content-Type: application/json" -X POST https://localhost:8866/accesshandler/signup
...
string byte_2_str(const unsigned char* bytes, int size) {
  string str;
...
void ecall_get_role(char * r, size_t len){ 
...
int ecall_signout(){
    ...
    if (it == SIGNED_USERS.end()){	
        ...
        if (itr != SIGNED_USERS.end()) SIGNED_USERS.erase(itr);
